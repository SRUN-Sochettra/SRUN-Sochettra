...
// សួស្តី (sua sdei). Codepoints: ស U+179F · ួ U+17BD (vowel UA) · ស U+179F ·
// ្ U+17D2 (coeng/virama) · ត U+178F · ី U+17B8 (vowel II).
// Previous value used U+17BC (vowel UU) which spelled សូស្តី — wrong word.
const KHMER_GREETING = "\u179F\u17BD\u179F\u17D2\u178F\u17B8";
...
async function getStats() {
  ...
  try {
    ...
    if (latestPush) {
      ...
      // Parse owner from "owner/repo" instead of assuming USER owns it.
      // Push events can come from repos you contribute to but don't own.
      ...
      let lang = "\u2014";
      try {
        const { data: repo } = await octo.repos.get({ owner, repo: repoName });
        lang = repo.language || "\u2014";
      } catch (err) {
        console.warn(`repo lookup failed for ${owner}/${repoName}:`, err.message);
      }
...
const [stats, anime, khmerB64, displayB64] = await Promise.all([
  ...
  // DISPLAY_NAME already contains every glyph needed; dead concat removed.
